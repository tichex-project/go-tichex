...
var (
	...
	defaultAmount                  = defaultTokens.String() + assets.MicroTichexDenom
...
func GenTxCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		Args:  cobra.NoArgs,
		Long: fmt.Sprintf(`This command is an alias of the 'tichexd tx create-validator' command'.
...
`, defaultAmount, defaultCommissionRate, defaultCommissionMaxRate, defaultCommissionMaxChangeRate, defaultMinSelfDelegation),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err = app.TichexValidateGenesisState(genesisState); err != nil {
				return err
			}
