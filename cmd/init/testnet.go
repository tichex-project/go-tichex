...
func TestnetFilesCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a tichexd testnet",
		Long: `testnet will create "v" number of directories and populate each with
necessary files (private validator, genesis, config, etc.).
Note, strict routability for addresses is turned off in the config file.
Example:
	tichexd testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "tichexd",
		"Home directory of the node's daemon configuration",
	)
	cmd.Flags().String(flagNodeCliHome, "tichexcli",
	...
	cmd.Flags().String(
		server.FlagMinGasPrices, fmt.Sprintf("0.015%s,0.015%s,0.015%s,0.015%s,0.015%s,0.015%s,0.015%s,0.015%s", assets.MicroTichexDenom, assets.MicroTHEURDenom),
...
func initTestnet(config *tmconfig.Config, cdc *codec.Codec) error {
	...
	tichexConfig := srvconfig.DefaultConfig()
	tichexConfig.MinGasPrices = viper.GetString(server.FlagMinGasPrices)
	...
	for i := 0; i < numValidators; i++ {
		...
		accs = append(accs, app.GenesisAccount{
			...
			Coins: sdk.Coins{
				...
				sdk.NewCoin(assets.MicroTichexDenom, accStakingTokens),
			},
		})
		...
		msg := staking.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(assets.MicroTichexDenom, valTokens),
		...
		tichexConfigFilePath := filepath.Join(nodeDir, "config/tichexd.toml")
		srvconfig.WriteConfigFile(tichexConfigFilePath, tichexConfig)
	}
