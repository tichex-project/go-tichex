...
import (
	...
	tichexInit "github.com/tichex-project/go-tichex/cmd/init"

	"github.com/cosmos/cosmos-sdk/server"
	sdk "github.com/cosmos/cosmos-sdk/types"
	tichexserver "github.com/tichex-project/go-tichex/server"
)
...
func main() {
	...
	rootCmd := &cobra.Command{
		Use:               "tichexd",
		Short:             "Tichex Blockchain Daemon (server)",
		PersistentPreRunE: tichexserver.PersistentPreRunEFn(ctx),
	}
	rootCmd.AddCommand(tichexInit.InitCmd(ctx, cdc))
	rootCmd.AddCommand(tichexInit.CollectGenTxsCmd(ctx, cdc))
	rootCmd.AddCommand(tichexInit.TestnetFilesCmd(ctx, cdc))
	rootCmd.AddCommand(tichexInit.GenTxCmd(ctx, cdc))
	rootCmd.AddCommand(tichexInit.AddGenesisAccountCmd(ctx, cdc))
	rootCmd.AddCommand(tichexInit.ValidateGenesisCmd(ctx, cdc))
...
func newApp(logger log.Logger, db dbm.DB, traceStore io.Writer) abci.Application {
	return app.NewTichexBlockchain(
...
func exportAppStateAndTMValidators(
	...
	if height != -1 {
		tApp := app.NewTichexBlockchain(logger, db, traceStore, false, false)
	...
	tApp := app.NewTichexBlockchain(logger, db, traceStore, true, false)
	return tApp.ExportAppStateAndValidators(forZeroHeight, jailWhiteList)
}