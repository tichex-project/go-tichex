...
func interceptLoadConfig() (conf *cfg.Config, err error) {
	...
	tichexConfigFilePath := filepath.Join(rootDir, "config/tichexd.toml")
	if _, err := os.Stat(tichexConfigFilePath); os.IsNotExist(err) {
		tichexConf, _ := config.ParseConfig()
		config.WriteConfigFile(tichexConfigFilePath, tichexConf)
	}

	viper.SetConfigName("tichexd")
